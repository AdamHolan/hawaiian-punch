...
class character(pygame.sprite.Sprite):
    ...
    #     if self.attacking:
    def shoot(self):
        self.newBullet = Bullet(self.rect.x,self.rect.y)
        self.bulletList.add(self.newBullet)
        print(self.bulletList)
        # pygame.draw.rect(screen,black,[self.rect.x+self.bulletSpeed,self.rect.y,10,10],)
        
        #draw it out
        #self.bulletList.draw(screen)
        #move it when a key is pressed
...
def fight(roundNum, player1Points, player2Points):
    ...
    player1.bulletList.draw(screen)
    playersList.update()
    fruitsList.update()
    for player in playersList:
